...
show_current_settings() {
  echo "󰒓 Current Settings"
  echo "─────────────────"
  echo "� Rices Directory: $RICES_DIR"
  echo "󰆵 Buffer Directory: $BUFFER_DIR"
  echo "󰆊 Buffer Size: $BUFFER_SIZE backups"
  echo "󰒓 Use Symlinks: $([[ "$USE_SYMLINKS" == "true" ]] && echo "󰄲 Enabled" || echo "󰄱 Disabled")"
  echo "󰒓 Auto Backup: $([[ "$AUTO_BACKUP" == "true" ]] && echo "󰄲 Enabled" || echo "󰄱 Disabled")"
  echo "󰒓 Backup on Switch: $([[ "$BACKUP_ON_SWITCH" == "true" ]] && echo "󰄲 Enabled" || echo "󰄱 Disabled")"
  echo "󰒓 Confirm Actions: $([[ "$CONFIRM_ACTIONS" == "true" ]] && echo "󰄲 Enabled" || echo "󰄱 Disabled")"
  echo "🎨 Theme: $THEME"
  echo "󰒓 Log Level: $LOG_LEVEL"
  echo "󰆊 Max Log Files: $MAX_LOG_FILES"
  echo "󰒓 Notifications: $([[ "$NOTIFICATIONS" == "true" ]] && echo "󰄲 Enabled" || echo "󰄱 Disabled")"
  echo "󰚌 Excluded Folders: ${EXCLUDED_FOLDERS[*]}"
}
